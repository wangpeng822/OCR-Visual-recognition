...
本项目可以按“低耦合、高内聚”的方式拆成多个可并行开发模块。只要前期先约定数据结构、信号接口、配置字段，大部分模块都可以独立推进，最后再统一联调。
...
1. 只依赖模型文件和输入图片。
2. 可以脱离 GUI 独立做命令行验证。
3. 可以先输出 json 结果，不必等待界面完成。

联调依赖：

1. 向上层暴露统一 `run(image_path) -> OCRResult` 接口。
2. 坐标必须与界面标注模块约定统一格式。
...
2. 展示完整 OCR 文本。
3. 提供手动编辑、复制、追加等操作。
4. 提供“设为待写入 NFC 编号”等联动入口。

可独立开发原因：

1. 可先基于模拟文本事件开发。
2. 不需要真实 OCR 或 NFC 才能完成界面和交互。

联调依赖：

1. 接收图片标注模块的文本选中事件。
2. 接收 NFC 模块的读卡结果并回填显示。
...
1. 可完全脱离 GUI 单独调试。
2. 可先实现 `MockNFCService`，后续再替换真实设备适配器。
...
这些模块在开发阶段都可以通过“模拟数据”或“假服务”先跑通。

## 19.4 需要提前约定的联调接口

为了让模块后期顺利拼接，必须提前统一以下接口。
...
```python
text_box_selected(box: OCRTextBox) -> None
```

右侧侧边栏只监听这个事件，不直接依赖图片组件内部实现。
...
这样前期可以接 `MockNFCService`，后期再替换真实实现。
...
这样图片列表、进度条、日志区都能共用同一状态定义。

## 19.5 推荐的联调顺序

推荐不要所有模块完成后一次性总联调，而是分三轮联调：
...
1. 右侧侧边栏 + NFC 服务模块
2. 确认当前选中文字可传入待写入区
3. 确认 NFC 读取、写入、回填链路正确

## 19.6 最终结论

本项目里最适合独立拆分、后期再联调的模块是：
...
1. `OCR` 和 `NFC` 最适合完全独立开发。
2. `UI 框架` 和 `右侧侧边栏` 也可以不等底层完成先做。
3. `图片标注点击模块` 只要拿到标准 OCR 坐标结构，就可以后期快速联调。
4. 真正需要最早统一的不是实现代码，而是 `OCRResult`、选中事件、NFCService 这三个接口边界。
